...
async function main() {
  ...
  if (!academicYear) {
    academicYear = await prisma.academicYear.create({ data: { year: '2569' } });
  }

  let term1 = await prisma.term.findFirst({
    where: { academicYearId: academicYear.id, name: '1/2569' },
  });
  if (!term1) {
  ...
  const qInstrument = await prisma.instrument.upsert({
    ...
    create: {
      code: 'Q-MODEL-2569',
      nameTh: 'แบบประเมิน Q-Model ปี 2569',
      nameEn: 'Q-Model Assessment 2569',
      type: 'Q_MODEL',
      version: '1.0',
      isActive: true,
    },
