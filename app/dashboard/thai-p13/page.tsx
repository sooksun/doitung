...
// Detailed THAI_P1_3 dashboard with drill-down: ภาพรวม → โรงเรียน → ครู (รายบุคคล).
// Shows ครูประเมินตนเอง (self) vs ผอ.ประเมิน (director) vs ค่าเป้าหมาย (target) on the
// 1–4 scale, per dimension + per indicator, with per-round progress + reflections.
...
export default function ThaiP13DashboardPage() {
  ...
  }, [token, academicYearId, level, schoolId, teacherId, router]);
  ...
  const th: React.CSSProperties = { border: '1px solid #e5e7eb', padding: '0.5rem 0.6rem', background: '#f8fafc', fontSize: '0.82rem', textAlign: 'center' };
  const td: React.CSSProperties = { border: '1px solid #e5e7eb', padding: '0.5rem 0.6rem', fontSize: '0.85rem' };
  ...
  const Breadcrumb = () => (
    <div style={{ fontSize: '0.9rem', color: '#6b7280', marginBottom: '1rem', display: 'flex', gap: '0.4rem', flexWrap: 'wrap', alignItems: 'center' }}>
      ...
      {(level === 'school' || level === 'teacher') && data?.school && (
        <>
          <span>›</span>
          <button onClick={() => schoolId && goSchool(schoolId)} style={{ background: 'none', border: 'none', color: level === 'school' ? '#111827' : '#6366f1', cursor: 'pointer', fontWeight: level === 'school' ? 700 : 500, padding: 0, fontSize: '0.9rem' }}>{data.school?.name || (level === 'teacher' ? data?.teacher?.schoolName : '')}</button>
        </>
      )}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', padding: '2rem' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1rem', margin: '0.5rem 0 1rem' }}>
          <div>
            <h1 style={{ fontSize: '1.55rem', fontWeight: 800, color: '#111827', margin: '0 0 0.2rem' }}>📊 แดชบอร์ดภาษาไทย ป.1–3</h1>
          ...
          <>
            ...
            {level === 'overview' && (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: '1.25rem', marginBottom: '0.25rem' }}>
                  ...
                  <div style={card}>
                    ...
                    {(data.reflections as ReflRow[]).map((rf, i) => (
                      <div key={i} style={{ borderLeft: `3px solid ${rf.round === 1 ? '#3b82f6' : '#a855f7'}`, padding: '0.4rem 0 0.4rem 0.8rem', marginBottom: '0.75rem' }}>
                        <div style={{ fontSize: '0.82rem', fontWeight: 700, color: rf.round === 1 ? '#1d4ed8' : '#7c3aed' }}>
                          {rf.sectionName} · ครั้งที่ {rf.round} {rf.recordedAt ? `· ${new Date(rf.recordedAt).toLocaleDateString('th-TH')}` : ''}
                        </div>
