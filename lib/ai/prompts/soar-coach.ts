...
export const SOAR_PROMPT_VERSION = 'soar-v2';
...
# Iceberg Model — มุมวิเคราะห์เพิ่มเติม (icebergByDimension)
นอกจาก SOAR แล้ว ให้สร้างมุมมอง Iceberg Model 4 ชั้น × 2 ด้าน (current = สิ่งที่เป็นอยู่ / desired = สิ่งที่อยากให้เป็น) ของแต่ละมิติ Q-Model:

ชั้น 1 — situations (สถานการณ์ที่เห็น):
  สิ่งที่กำลังเกิดขึ้นในโรงเรียนตอนนี้ — เหตุการณ์ที่สังเกตได้ ทั้งเชิงบวกและเชิงลบ + แนวโน้มจากบริบทภายนอก (สังคม นโยบายการศึกษา) ที่เกี่ยวข้อง

ชั้น 2 — patterns (รูปแบบของปัญหา/สิ่งที่เกิดซ้ำ):
  อะไรเป็นเรื่องที่เกิดซ้ำๆ จนกลายเป็นรูปแบบ — ดึงจากเหตุการณ์ใน situations แล้วยกระดับให้เห็น "วงจร" หรือ "เทรนด์" ที่หล่อเลี้ยงสภาพปัจจุบัน

ชั้น 3 — structures (โครงสร้าง):
  อะไรคือกลไก/ระบบ/นโยบาย/ทรัพยากรที่ทำให้รูปแบบนั้นยังคงอยู่ — เช่น ตารางสอน ภาระงานครู โครงสร้างการบริหาร งบประมาณ ความสัมพันธ์กับชุมชน เกณฑ์ประเมิน

ชั้น 4 — mentalModels (แบบจำลองวิธีคิด):
  ความเชื่อ ค่านิยม ทัศนคติ สมมติฐานที่ฝังลึก ที่ทำให้โครงสร้างนั้นยังถูกยอมรับและสืบทอด — เป็นชั้นที่ลึกที่สุด ส่งผลย้อนกลับขึ้นมาทุกชั้น

แต่ละชั้นกรอก 2 ด้าน:
- current: สิ่งที่เป็นอยู่จริง อิงจากคะแนน + SAR (1-5 bullet)
- desired: สิ่งที่อยากให้เป็น สอดคล้องกับ desired score + Aspirations (1-5 bullet)
...
9. Iceberg ทุกชั้นต้องสอดคล้องกัน: situations → patterns → structures → mentalModels เป็นการลงลึกจากสิ่งที่เห็นไปหาความเชื่อ (ห้ามใส่ความเชื่อในชั้น situations)
